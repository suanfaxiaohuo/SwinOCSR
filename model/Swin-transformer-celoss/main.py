...
def parse_option():
    ...
    parser.add_argument('--cfg', default='../configs/swin_large_patch4_window7_224.yaml',type=str, metavar="FILE", help='path to config file', )
    parser.add_argument(
        "--opts",
    ...
    parser.add_argument("--test_dir", default='../../Data/500wan/500wan_shuffle_DeepSMILES_test.pkl', type=str, help='direction for eval_dataset')
...
def main(config) :
    ...
    encoder = build_model(config, tag=False)
    encoder.cuda()
...
def train_one_epoch(config, encoder, encoder_optimizer, decoder, decoder_optimizer, criterion, train_loader, epoch, mixup_fn, encoder_lr_scheduler,scaler):
    ...
    for idx, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        for index, caplen in enumerate(caplens_np):
            caps_input.append(caps_np[index][:caplen - 1] + caps_np[index][caplen:])  # <end> is removed  [[],[]]
            caps_target += caps_np[index][
                           1:caplen]  # <start> and the extra pad after it were removed
        ...
        decode_lengths = np.array(decode_lengths)  # Do not include the < start >
        tgt_pad_mask = make_std_mask(caps_input, pad=0)  # Genetate mask
